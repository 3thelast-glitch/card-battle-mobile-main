...
const RARITIES: {
  ...
  {
    id: 'Common', label: 'Common', labelAr: 'عادي',
    gradient: ['#064e3b', '#065f46'], border: '#10b981', textColor: '#34d399',
    badgeBg: 'rgba(16,185,129,0.15)', checkColor: '#10b981', glowColor: 'rgba(16,185,129,0.25)',
  },
  {
    id: 'Rare', label: 'Rare', labelAr: 'نادر',
    gradient: ['#1e3a5f', '#1e40af'], border: '#3b82f6', textColor: '#93c5fd',
    badgeBg: 'rgba(59,130,246,0.15)', checkColor: '#3b82f6', glowColor: 'rgba(59,130,246,0.25)',
  },
  {
    id: 'Epic', label: 'Epic', labelAr: 'ملحمي',
    gradient: ['#3b0764', '#6d28d9'], border: '#a855f7', textColor: '#d8b4fe',
    badgeBg: 'rgba(168,85,247,0.15)', checkColor: '#a855f7', glowColor: 'rgba(168,85,247,0.3)',
  },
  {
    id: 'Legendary', label: 'Legendary', labelAr: 'أسطوري',
    gradient: ['#78350f', '#b45309'], border: '#f59e0b', textColor: '#fcd34d',
    badgeBg: 'rgba(245,158,11,0.15)', checkColor: '#f59e0b', glowColor: 'rgba(245,158,11,0.35)',
  },
  {
    id: 'Special', label: 'Special', labelAr: 'خاص ✦',
    gradient: ['#0f172a', '#701a75'], border: '#e879f9', textColor: '#f0abfc',
    badgeBg: 'rgba(232,121,249,0.18)', checkColor: '#e879f9', glowColor: 'rgba(232,121,249,0.45)',
  },
];

// ─── فتح معرض الصور حسب المنصة ───
const pickImageCrossPlatform = (onPick: (uri: string) => void) => {
  if (Platform.OS === 'web') {
    // الويب: input[file] مخفي
    ...
    // موبايل: استخدم Alert كبديل حتى يتم تثبيت expo-image-picker
    Alert.alert(
      'اختيار صورة',
      'لاحظة: خاصية الصورة تعمل على الويب حالياً. لتفعيلها على الموبايل ثبت: pnpm add expo-image-picker',
      [{ text: 'حسناً' }]
    );
  }
};
...
export default function EditAbilityScreen() {
  ...
  const handlePickImage = () => pickImageCrossPlatform((uri) => setImageUri(uri));
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            ...
            {imageUri ? (
              <View style={styles.imagePreviewWrapper}>
                <Image
                  source={{ uri: imageUri }}
                  style={[styles.imagePreview, { borderColor: selectedRarity.border }]}
                  resizeMode="cover"
                />
                <View style={styles.imageActions}>
                  ...
                  >
                    ...
                    <ThemedText style={[styles.imageActionText, { color: selectedRarity.textColor }]}>تغيير</ThemedText>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={handleRemoveImage} style={styles.imageRemoveBtn} activeOpacity={0.75}>
              ...
              <TouchableOpacity
                ...
                style={[
                  ...
                  { borderColor: selectedRarity.border + '60', backgroundColor: selectedRarity.badgeBg },
                ]}
              >
                <View style={[styles.imageUploadIcon, { backgroundColor: selectedRarity.badgeBg, borderColor: selectedRarity.border + '80' }]}>
                  <ImagePlus size={28} color={selectedRarity.textColor} />
                </View>
                <ThemedText style={[styles.imageUploadTitle, { color: selectedRarity.textColor }]}>اختر صورة الكرت</ThemedText>
                <ThemedText style={styles.imageUploadSub}>اضغط لرفع صورة من المعرض</ThemedText>
              </TouchableOpacity>
            )}
          </View>
          ...
          {/* ─── INPUTS ─── */}
          <View style={styles.section}>
            <InputField label="English Name" value={nameEn} onChangeText={setNameEn} placeholder="e.g. Logical Encounter" textAlign="left" accentColor={selectedRarity.textColor} />
            <InputField label="الاسم العربي" value={nameAr} onChangeText={setNameAr} placeholder="مثال: مصادفة منطقية" textAlign="right" accentColor={selectedRarity.textColor} />
            <InputField label="الوصف / Description" value={description} onChangeText={setDescription} placeholder="وصف القدرة..." multiline textAlign="right" accentColor={selectedRarity.textColor} />
            <InputField label="⚠️ تحذير / Warning" value={descriptionWarning} onChangeText={setDescriptionWarning} placeholder="تحذير أو ملاحظة اختيارية..." multiline textAlign="right" accentColor="#f87171" />
          </View>
          ...
          <View style={styles.section}>
            ...
            <View
              style={[
                styles.previewCard,
                { borderColor: selectedRarity.border, shadowColor: selectedRarity.glowColor, backgroundColor: selectedRarity.gradient[1] + '33' },
              ]}
            >
              {imageUri && (
                <Image source={{ uri: imageUri }} style={[styles.previewImage, { borderColor: selectedRarity.border + '60' }]} resizeMode="cover" />
              )}
              <View style={styles.previewBadge}>
                <View style={[styles.previewBadgePill, { backgroundColor: selectedRarity.badgeBg, borderColor: selectedRarity.border + '80' }]}>
                  <ThemedText style={[styles.previewBadgeText, { color: selectedRarity.textColor }]}>{selectedRarity.labelAr}</ThemedText>
                </View>
              </View>
              <ThemedText style={[styles.previewName, { color: selectedRarity.textColor }]}>{nameAr || nameEn || '—'}</ThemedText>
              {description ? <ThemedText style={styles.previewDesc}>{description}</ThemedText> : null}
              {descriptionWarning ? <ThemedText style={styles.previewWarning}>⚠️ {descriptionWarning}</ThemedText> : null}
            </View>
          </View>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity
          onPress={handleSave}
          style={[styles.saveBtn, { backgroundColor: selectedRarity.border, shadowColor: selectedRarity.glowColor }]}
        >
...
const styles = StyleSheet.create({
  ...
  sectionLabel: {
    ...
    textTransform: 'uppercase', letterSpacing: 1.2, marginBottom: 12, textAlign: 'right',
  },
  rarityRow: { flexDirection: 'row', gap: 10 },
  rarityCard: {
    flex: 1, minHeight: 80, borderRadius: 14, borderWidth: 1.5, padding: 12, justifyContent: 'space-between',
  },
  ...
  imagePreview: { width: '100%', height: 220, borderRadius: 18, borderWidth: 2 },
  imageActions: { flexDirection: 'row', gap: 10, width: '100%' },
  imageActionBtn: {
  ...
  // Preview
  previewCard: {
    borderWidth: 2, borderRadius: 18, padding: 18,
    shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.8, shadowRadius: 16, elevation: 8, overflow: 'hidden',
  },
  previewImage: { width: '100%', height: 160, borderRadius: 12, marginBottom: 12, borderWidth: 1 },
  previewBadge: { alignItems: 'flex-end', marginBottom: 10 },
  previewBadgePill: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, borderWidth: 1 },
  ...
  inputBox: {
    ...
    borderColor: 'rgba(255,255,255,0.1)', borderRadius: 14, paddingHorizontal: 14, paddingVertical: 12,
  },
  ...
  footer: {
    ...
    backgroundColor: 'rgba(2,6,23,0.95)', borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.08)',
  },
  cancelBtn: {
    flex: 1, height: 54, borderRadius: 14, borderWidth: 1,
  ...
  saveBtn: {
    flex: 2, height: 54, borderRadius: 14, alignItems: 'center', justifyContent: 'center',
    flexDirection: 'row', gap: 8, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.6, shadowRadius: 12, elevation: 8,
  },
