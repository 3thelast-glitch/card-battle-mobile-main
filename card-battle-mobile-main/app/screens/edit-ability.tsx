...
// ──────────────────────────────────────────────────────────────
// رارتي config — تشمل الآن Special
// ──────────────────────────────────────────────────────────────
const RARITIES: {
  id: Rarity;
  label: string;
...
export default function EditAbilityScreen() {
  ...
  const handleSave = () => {
    console.log('Saving:', { nameEn, nameAr, description, descriptionWarning, rarity });
    navigation.goBack();
  };
      ...
      >
        ...
        >
          ...
          <View style={styles.section}>
            ...
            <View style={styles.rarityRow}>
              {RARITIES.slice(0, 2).map((r) => (
                <RarityCard
                  key={r.id}
                  r={r}
                  isSelected={rarity === r.id}
                  onPress={() => setRarity(r.id)}
                />
              ))}
            </View>

            {/* Row 2: Epic + Legendary */}
            ...
            {/* Row 3: Special — full width */}
            <View style={[styles.rarityRow, { marginTop: 10 }]}>
              <SpecialRarityCard
                r={RARITIES[4]}
                isSelected={rarity === 'Special'}
                onPress={() => setRarity('Special')}
              />
            </View>
          </View>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity
          ...
          style={[
            styles.saveBtn,
            {
              backgroundColor: selectedRarity.border,
              shadowColor: selectedRarity.glowColor,
            },
          ]}
        >
