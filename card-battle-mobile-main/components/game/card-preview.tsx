        ...
        <Animated.View style={[animatedStyle, style, !isActive && { opacity: 0.5 }]}>
            <View className={!isActive ? "grayscale" : ""}>
                <Pressable
                    ...
                    // Hover lift (works on web/pointer)
                ...
                {__DEV__ && (
                    <View className="absolute top-4 left-4 right-4 z-50 flex-row justify-between pointer-events-auto">
                        <TouchableOpacity 
                            ...
                            className={`w-8 h-8 rounded-full items-center justify-center border ${!isActive ? 'bg-red-500/20 border-red-500/50' : 'bg-black/80 border-white/20'}`}
                        >
