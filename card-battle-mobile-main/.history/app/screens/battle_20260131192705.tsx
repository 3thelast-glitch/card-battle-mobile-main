...
// ✅ Component قابل للتحريك مع التقاط للشبكة
...
export default function BattleScreen() {
  ...
  const [showGrid, setShowGrid] = useState(true); // ✅ التحكم بالشبكة
  const [snapToGrid, setSnapToGrid] = useState(false); // ✅ التقاط للشبكة
      ...
      <View style={styles.mainControls}>
        ...
        {editMode && (
          <>
            ...
            <TouchableOpacity
              style={[styles.gridToggleButton, showGrid && styles.gridToggleButtonActive]}
              onPress={() => setShowGrid(!showGrid)}
            >
              <Text style={styles.gridToggleButtonText}>
                {showGrid ? '👁️ إخفاء الشبكة' : '👁️‍🗨️ إظهار الشبكة'}
              </Text>
            </TouchableOpacity>

            {/* ✅ زر التقاط للشبكة */}
            <TouchableOpacity
              style={[styles.snapToggleButton, snapToGrid && styles.snapToggleButtonActive]}
              onPress={() => setSnapToGrid(!snapToGrid)}
            >
              <Text style={styles.snapToggleButtonText}>
                {snapToGrid ? '🧲 التقاط: تشغيل' : '🧲 التقاط: إيقاف'}
              </Text>
            </TouchableOpacity>
          </>
        )}
      ...
      {editMode && (
        <View style={styles.editInstructions}>
          ...
          <Text style={styles.editInstructionsSubText}>
            🔒 الحدود محمية • 📐 الشبكة تساعد في التوزيع المتوازن
          </Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  snapToggleButtonActive: {
    backgroundColor: '#4ade80',
  },

  snapToggleButtonText: {
    color: '#fff',
    fontSize: 13,
    fontWeight: 'bold',
  },
