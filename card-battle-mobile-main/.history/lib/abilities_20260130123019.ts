...
export function executeAbility(
  ...
  if (!executor) {
    console.warn(`No executor found for ability: ${card.ability} - abilities.ts:250`);
    return { newEffects: [] };
  }
  ...
  if (sealEffect) {
    console.log(`Ability ${card.ability} is sealed for ${isPlayer ? 'player' : 'bot'} - abilities.ts:260`);
    return { newEffects: [] };
  }
