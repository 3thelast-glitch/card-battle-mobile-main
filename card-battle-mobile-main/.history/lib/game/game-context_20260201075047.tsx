...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case 'USE_ABILITY': {
      ...
      // إضافة رابط الفيديو للبطاقة الحالية إذا كانت القدرة تدعم الفيديو
      const abilityVideoUrls: Record<string, string> = {
        'Protection': 'https://d23dyxeqlo5psv.cloudfront.net/big_buck_bunny.mp4',
        'Seal': 'https://d23dyxeqlo5psv.cloudfront.net/big_buck_bunny.mp4',
        'Eclipse': 'https://d23dyxeqlo5psv.cloudfront.net/big_buck_bunny.mp4',
      };

      if (abilityVideoUrls[abilityType]) {
        if (isPlayer) {
          const newPlayerDeck = [...state.playerDeck];
          newPlayerDeck[state.currentRound] = {
            ...newPlayerDeck[state.currentRound],
            videoUrl: abilityVideoUrls[abilityType]
          };
          newState.playerDeck = newPlayerDeck;
        } else {
          const newBotDeck = [...state.botDeck];
          newBotDeck[state.currentRound] = {
            ...newBotDeck[state.currentRound],
            videoUrl: abilityVideoUrls[abilityType]
          };
          newState.botDeck = newBotDeck;
        }
      }
